...
public class WorkbenchRecipes {
    /**
     * Registers workbench crafting recipes and writes them to the given exporter.
     *
     * Defines workbench recipes (armorer, furnace T1, farmer, builder) with their inputs,
     * outputs, crafting times and book category, and saves them. Unlock conditions use
     * the provider to check for the base workbench; item tag inputs are resolved via the lookup.
     *
     * @param provider provider used to query existing items/blocks for unlock conditions (e.g. presence of the base workbench)
     * @param exporter destination for the generated recipe data
     * @param lookup   holder lookup used to resolve item tag inputs when building recipes
     */
